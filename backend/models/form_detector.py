...
class FormDetector:
    def __init__(self):
        self.suspicious_form_indicators = [
            'password', 'login', 'signin', 'username', 'email', 'credit',
            'cvv', 'card', 'ssn', 'social'
        ]
    ...
    def analyze_form(self, form, page_url):
        ...
        suspicious_inputs = [
            ...
            if any(indicator in name for indicator in self.suspicious_form_indicators)
        ]
        ...
        # Check for missing CSRF protection (look for typical CSRF token names)
        csrf_tokens = form.find_all('input', {'type': 'hidden'})
        csrf_names = ['csrf', 'token', 'authenticity_token', 'csrfmiddlewaretoken']
        has_csrf = any(
            any(csrf_name in inp.get('name', '').lower() for csrf_name in csrf_names)
            for inp in csrf_tokens
        )
        if not has_csrf and 'password' in input_types:
